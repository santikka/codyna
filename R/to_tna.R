...
#'
#' * `resilience`: Six-level resilience categories (Excellent to Troubled)
#'   from `classify_resilience()`. Available for the composite score and
#'   each individual metric.
#' * `hurst`: Five Hurst memory states (strong_antipersistent to
#'   strong_persistent), or four multifractal categories when using MFDFA.
#' * `hurst_ews`: Five warning levels (none to critical) from
#'   `detect_hurst_warnings()`.
#' * `multi_ews`: System states (Stable to Failing) from expanding-window
#'   multivariate EWS. Rolling-window objects have no state classification
#'   and are not supported.
#' * `trend`: Four trend states (ascending, descending, flat, turbulent)
#'   from `compute_trend()`.
#' * `changepoint`: Regime state labels combining level and changepoint
...
#'   `"regime"`, or `"segment"`.
#' * `spectral`: Four noise-colour states (white_noise, pink_noise,
#'   red_noise, brownian) from `spectral_ews()` with `states = TRUE`.
#' * `potential`: Stability states (unimodal, bimodal, multimodal) derived
...
to_tna.multi_ews <- function(x, ...) {
  ...
  stopifnot_(
    !is.null(method) && method == "expanding",
  ...
  stopifnot_(
    !is.null(cls) && "state" %in% names(cls),
...
to_tna.trend <- function(x, ...) {
  stopifnot_(
    "state" %in% names(x),
    "The {.cls trend} object has no {.val state} column."
  )
  states <- as.character(x$state)
  seq_to_wide_(states, label = "state")
}
...
to_tna.changepoint <- function(x, state = "state", ...) {
  ...
  if (state == "regime") {
    ...
    if (!state %in% names(x)) {
      stop_(
        "Column {.val {state}} not found. Available: {.or {.val {valid}}}."
      )
    }
    states <- as.character(x[[state]])
  }
...
to_tna.spectral <- function(x, ...) {
  stopifnot_(
    "state" %in% names(x),
    "The {.cls spectral} object has no {.val state} column.
...
seq_to_wide_ <- function(states, label = "column") {
  ...
  stopifnot_(
    length(states) > 0L,
    "The {.val {label}} contains only NA values, no states to extract."
  )
