...
#'
#' * Stable: An AR(1) process with a low autoregressive coefficient
...
#' * Shock + Recovery: A sharp linear drop to 70% below baseline
...
#' * Volatile: An AR(1) process with the same autoregressive
#'     coefficient as Stable but much larger noise (sd = 15), producing
#'     high-amplitude fluctuations around the baseline.
#' * Turbulent: A random walk (cumulative sum of Gaussian increments,
...
#'
#' * If `generate_plot = TRUE`, a `list` with:
#'     * `data`: A `data.frame` with columns `time`, `value`,
#'       `true_phase`, and optionally `id` (when `n_individuals > 1`).
#'         Clustered mode also includes `quantile_class`.
#'     * `plots`: A `ggplot` or `patchwork` object showing the time
#'         series with classification bands.
#' * If `generate_plot = FALSE`, the `data.frame` directly.
#'
#'
#' @seealso [compare_ts()] for visualizing classification results,
...
#' out$plot
#' }
generate_ts_data <- function(n_individuals = 1L, data_length = 500L,
                             data_type = "resilience", n_stable_levels = 3L,
                             mean_stable_value = 100, level_separation = 20,
                             n_levels = 3L, results_df = NULL,
                             results_col = NULL, seed = 42L,
                             generate_plot = TRUE) {
  set.seed(seed)
  ...
  stopifnot_(
    data_type != "clustered" || n_levels %in% 2:6
  )
  stopifnot_(
    is.null(results_df) || !is.null(results_col),
    "When {.arg results_df} is provided, {.arg results_col} is required."
  )
  all_data <- lapply(
    ...
    function(i) {
      ...
      df <- ifelse_(
        data_type == "clustered",
        datagen_clustered_(
          data_length, n_levels, 100, 2, 1, NULL, 0.1, TRUE
        ),
        datagen_resilience_(
          data_length,
          n_stable_levels,
  ...
  final <- do.call(base::rbind, all_data)
  final$id <- as.factor(final$id)
  if (!generate_plot) {
    return(final)
  }
  fb <- onlyif(n_individuals > 1L, "id")
...
#' @param seed \[`integer(1)`: `0`\]\cr
#'   Random seed for reproducibility.
#'
#' @return A `data.frame` with columns:
#'
#'   * `Time`: integer time index from 1 to `n_time`.
#'   * `VAR1`, `VAR2`, \ldots, `VARn`: simulated variable values,
#'     where \eqn{n} = `n_vars`.
#'
...
#' )
#' head(tp_ar)
generate_tipping_data <- function(n_time = 200L, n_vars = 5L,
                                  tipping_point = 100L,
...
#'
#' Visualizes one or two classification results as colored background
...
#' drawn as a black line with classification labels shown as colored
#' background rectangles spanning the full y-axis range. Colors are
...
#' @return
#'
#'   * When `state2` is `NULL`: a single `ggplot` object.
#'   * When `state2` is provided: a `patchwork` object containing two
...
#' print(p2)
#' }
compare_ts <- function(data, data2 = NULL, ts_col, time_col = NULL,
                       state1, state2 = NULL, facet_by = NULL,
                       title1 = "Classification 1", title2 = "Classification 2",
                       palette1 = "Set2", palette2 = "Set3") {
  check_missing(data)
  ...
  if (is.null(time_col)) {
    message_(
      "Argument {.arg time_col} not provided.
      Inferring time index from row order."
    )
  ...
  if (!is.null(state2)) {
    if (!is.null(data2)) {
      check_class(data2, "data.frame")
      stopifnot_(
        state2 %in% names(data2),
        "Column {.var {state2}} not found in {.arg data2}."
      )
      stopifnot_(
        nrow(plot_df) == nrow(data2),
        "{.arg data} and {.arg data2} must have the same number of rows."
      )
      plot_df[[state2]] <- data2[[state2]]
    } else {
      stopifnot_(
        state2 %in% names(plot_df),
        "Column {.var {state2}} not found in {.arg data}."
      )
    }
  }
  cols_needed <- c(ts_col, time_col, state1, state2, facet_by)
  mis <- cols_needed[!cols_needed %in% names(plot_df)]
  stopifnot_(
    length(mis) == 0L,
    "Missing columns: {.var {paste(missing, collapse = ', ')}}."
  )
...
datagen_cluster_proportions_ <- function(n_levels, proportions,
  ...
  if (!is.null(proportions)) {
    stopifnot_(
      length(proportions) == n_levels,
      "Length of {.arg proportions} must equal {.arg n_levels}."
    )
    stopifnot_(
      abs(sum(proportions) - 1) <= 1e-10,
      "Argument {.arg proportions} must sum to 1."
    )
    return(proportions)
  }
  ...
  for (attempt in seq_len(100L)) {
    ...
    w <- w / sum(w)
    if (all(w >= min_cluster_size)) {
      return(w)
    }
  }
  warning_(
    "Could not satisfy constraints after 100 attempts. Using equal proportions."
  )
...
datagen_resilience_ <- function(data_length, n_stable_levels,
  ...
  check_values(n_stable_levels, strict = TRUE)
  if (n_stable_levels == 1L) {
    ...
    if (n_stable_levels == 2L) {
      stable_levels <- mean_stable_value + c(-1, 1) * offset
      stable_names  <- c("Stable Low", "Stable High")
    } else if (n_stable_levels == 3L) {
      stable_levels <- mean_stable_value + c(-1, 0, 1) * offset
  ...
  all_levels <- unique(
    c(stable_names, "Shock", "Recovery", "Volatile", "Turbulent")
  )
  data.frame(
...
datagen_clustered_ <- function(data_length, n_levels, base_value,
  ...
  configs <- list(
    ...
    "4" = list(
      nm = c("Very Low", "Low", "High", "Very High"),
      m  = c(-2, -0.7, 0.7, 2) * cluster_sep),
    "5" = list(
      nm = c("Very Low", "Low", "Medium", "High", "Very High"),
