# --------------------------------------------------------------------------
# test-potential.R -- Tests for potential_analysis()
# --------------------------------------------------------------------------
...
test_that("summary.potential does not error", {
  pa <- potential_analysis(bimodal)
  summary(pa) |>
    capture.output() |>
    expect_error(NA)
...
# --- Internal helper: potential_find_minima_ edge cases (lines 22, 28) -------

test_that("potential_find_minima_ returns empty for short vector", {
  minima_fn <- codyna:::potential_find_minima_
  # Line 22: n < 3 returns integer(0)
  expect_length(minima_fn(c(1, 2)), 0L)
  expect_length(minima_fn(numeric(0)), 0L)
  expect_length(minima_fn(5), 0L)
})

test_that("potential_find_minima_ handles zero sign forward-filling", {
  minima_fn <- codyna:::potential_find_minima_
  # Line 28: sign_d[i] == 0 && i > 1 => forward-fill
...
# --- Internal helper: potential_find_maxima_ edge cases (lines 44, 49) -------

test_that("potential_find_maxima_ returns empty for short vector", {
  maxima_fn <- codyna:::potential_find_maxima_
  # Line 44: n < 3 returns integer(0)
  expect_length(maxima_fn(c(1, 2)), 0L)
  expect_length(maxima_fn(numeric(0)), 0L)
})

test_that("potential_find_maxima_ handles zero sign forward-filling", {
  maxima_fn <- codyna:::potential_find_maxima_
  # Line 49: sign_d[i] == 0 && i > 1 => forward-fill
...
# --- Internal helper: potential_landscape_ with bandwidth (line 68) ----------

test_that("potential_landscape_ respects bandwidth parameter", {
  landscape_fn <- codyna:::potential_landscape_
  set.seed(42)
  x <- rnorm(200)
  # Line 68: bandwidth is not NULL
  result <- landscape_fn(x, n_bins = 50L, bandwidth = 0.5)
  expect_true(is.list(result))
  expect_true("landscape" %in% names(result))
  expect_true("wells" %in% names(result))
})

# --- Internal helper: potential_landscape_ global min fallback (lines 89-90) -

test_that("potential_landscape_ uses global min when no interior minimum", {
  landscape_fn <- codyna:::potential_landscape_
  # Lines 89-90: no interior minimum found, fallback to global minimum
  # Unimodal data with very tight distribution = single potential well
  set.seed(42)
  x <- rnorm(500, 0, 0.3)
  result <- landscape_fn(x, n_bins = 20L, bandwidth = NULL)
  expect_true(result$n_wells >= 1L)
})

# --- potential_analysis: bandwidth validation (line 335) ---------------------
...
test_that("potential_analysis rejects negative bandwidth", {
  # Line 335: check_range on bandwidth
  expect_error(potential_analysis(bimodal, bandwidth = -1))
})

# --- potential_analysis: rolling window edge cases (lines 378-384) -----------

test_that("rolling window produces non-NA n_wells for valid windows", {
  set.seed(42)
  x <- rnorm(150)
  # Lines 378-379: windows with too few clean observations get NA
...
# --- plot.potential: landscape with rolling (line 628) -----------------------

test_that("plot.potential landscape type with rolling appends panel", {
  set.seed(42)
  pa <- potential_analysis(bimodal, window = 50L)
  # Line 628: landscape type with rolling present does NOT early-return
  p <- plot(pa, type = "landscape")
  # Should produce a patchwork (landscape + rolling panels)
  expect_true(inherits(p, "gg") || inherits(p, "patchwork"))
  # Force rendering to execute lazy closures (e.g., breaks function at line 628)
  grDevices::pdf(tempfile())
  suppressWarnings(print(p))
  grDevices::dev.off()
})

# --- plot.potential: density type returns single panel (line 645) ------------
...
# --- plot.potential: both type with rolling -----------------------------------
...
# --- print.potential: rolling section (lines 693-697) ------------------------

test_that("print.potential shows rolling info when present", {
  set.seed(42)
  pa <- potential_analysis(bimodal, window = 50L)
  # Lines 693-697: rolling section in print output
  out <- capture.output(print(pa))
  out_str <- paste(out, collapse = "\n")
  expect_true(grepl("Rolling", out_str) || grepl("rolling", out_str) ||
...
# --- summary.potential: rolling section (lines 760-772) ----------------------

test_that("summary.potential shows rolling section when present", {
  set.seed(42)
  pa <- potential_analysis(bimodal, window = 50L)
  # Lines 760-772: rolling-window summary section
  out <- capture.output(s <- summary(pa))
  out_str <- paste(out, collapse = "\n")
  expect_true(grepl("Rolling", out_str) || grepl("rolling", out_str) ||
              grepl("Window size", out_str))
  expect_true(grepl("Min wells", out_str))
  expect_true(grepl("Max wells", out_str))
  expect_true(grepl("Well-count changes", out_str) ||
              grepl("Mean wells", out_str))
  # Return value should include window
  expect_equal(s$window, 50L)
})

test_that("summary.potential shows barrier details when present", {
  set.seed(42)
  clear_bimodal <- c(rnorm(200, -3, 0.5), rnorm(200, 3, 0.5))
  pa <- potential_analysis(clear_bimodal)
  out <- capture.output(s <- summary(pa))
...
# --- potential_analysis: ts object input -------------------------------------
...
# --- potential_landscape_: no interior minima -> global min fallback (lines 89-90) ---

test_that("potential_landscape_ falls back to global min when no interior minimum", {
  # Lines 89-90: when potential_find_minima_ returns integer(0),
  ...
  set.seed(42)
  x <- rnorm(100)
  result <- landscape_fn(x, n_bins = 50L, bandwidth = NULL)
  # Should find exactly 1 well via global minimum fallback
  expect_equal(result$n_wells, 1L)
  expect_true(nrow(result$wells) == 1L)
})

# --- Rolling window: w_clean < 20 (lines 378-379) ---

# --- Lines 378-379: w_clean < 20 in rolling loop ---
...
# --- Lines 383-384: potential_landscape_ returns try-error in rolling ---
...
# --- plot.potential: line 645 is a safety guard (length(panels) == 1L) ---
# Analysis shows this line is unreachable with current code paths:
# - type "density" returns early at line 541
# - type "landscape" without rolling returns early at line 611
# - type "landscape" with rolling produces 2 panels (landscape + rolling)
# - type "both" always produces 2+ panels (density + landscape +/- rolling)
# Line 645 is dead code (defensive guard). No test needed.
