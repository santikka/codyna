# --- Test data ---------------------------------------------------------------
set.seed(42)
cp_data <- c(rnorm(100, 0, 1), rnorm(100, 5, 1))

# --- Basic output structure --------------------------------------------------

test_that("detect_cpts returns a changepoint tibble", {
  result <- detect_cpts(cp_data)
...
test_that("attributes are stored correctly", {
  ...
  result <- detect_cpts(cp_data, method = "pelt", penalty = "bic")
...
test_that("detect_cpts finds changepoint near known shift", {
  result <- detect_cpts(cp_data, method = "pelt")
...
test_that("cusum method works", {
  result <- detect_cpts(cp_data, method = "cusum")
  expect_s3_class(result, "changepoint")
  expect_true(attr(result, "n_changepoints") >= 1L)
})

test_that("binary_segmentation method works", {
  result <- detect_cpts(cp_data, method = "binary_segmentation")
  expect_s3_class(result, "changepoint")
  expect_true(attr(result, "n_changepoints") >= 1L)
})
...
test_that("type mean detects mean shift", {
  result <- detect_cpts(cp_data, type = "mean")
  expect_s3_class(result, "changepoint")
  expect_true(attr(result, "n_changepoints") >= 1L)
})
...
test_that("type both works", {
  result <- detect_cpts(cp_data, type = "both")
  expect_s3_class(result, "changepoint")
  expect_true(attr(result, "n_changepoints") >= 1L)
})
...
test_that("bad method produces error", {
  expect_error(
    detect_cpts(cp_data, method = "nonexistent"),
    "method"
  )
})
...
test_that("bad type produces error", {
  expect_error(
    detect_cpts(cp_data, type = "nonexistent"),
    "type"
  )
})
...
test_that("bad penalty produces error", {
  expect_error(
    detect_cpts(cp_data, penalty = "nonexistent"),
    "penalty"
  )
})

# --- Internal helper: changepoint_cost_ edge cases (lines 24, 37, 44, 48) ---

test_that("changepoint_cost_ returns 0 for segment with fewer than 2 obs", {
  cost_fn <- codyna:::changepoint_cost_
  # Line 24: n < 2 returns 0

  expect_equal(cost_fn(5, "mean"), 0)
  expect_equal(cost_fn(numeric(0), "mean"), 0)
})

test_that("changepoint_cost_ variance type handles near-zero variance", {
  cost_fn <- codyna:::changepoint_cost_
  # Line 37: v < eps guard for variance type

  result <- cost_fn(rep(3.0, 10), "variance")
  expect_true(is.finite(result))
})

test_that("changepoint_cost_ both type handles near-zero variance", {
  cost_fn <- codyna:::changepoint_cost_
  # Line 44: v < eps guard for both type
  result <- cost_fn(rep(3.0, 10), "both")
  expect_true(is.finite(result))
})

test_that("changepoint_cost_ default branch falls through", {
  cost_fn <- codyna:::changepoint_cost_
  # Line 48: default branch (unrecognized type)
  result <- cost_fn(c(1, 2, 3, 4), "unknown_type")
  # Default computes sum((x - mu)^2) = RSS
  expected <- sum((c(1, 2, 3, 4) - mean(c(1, 2, 3, 4)))^2)
  expect_equal(result, expected)
})

# --- Internal helper: changepoint_penalty_ edge cases (lines 70, 74) --------

test_that("changepoint_penalty_ manual with non-numeric value errors", {
  pen_fn <- codyna:::changepoint_penalty_
  # Line 70: manual penalty with NULL or non-numeric penalty_value
  expect_error(pen_fn("manual", NULL, 100, "mean"))
  expect_error(pen_fn("manual", "abc", 100, "mean"))
})

test_that("changepoint_penalty_ default branch returns BIC-like penalty", {
  pen_fn <- codyna:::changepoint_penalty_
  # Line 74: default switch branch (unrecognized penalty string)
  result <- pen_fn("unknown_penalty", NULL, 100, "mean")
  # Default: p * log(n) where p = 2 for type != "both"
  expect_equal(result, 2 * log(100))
})

# --- Internal helper: changepoint_best_split_ edge cases (lines 92, 109) ----

test_that("changepoint_best_split_ returns NULL for short segment", {
  split_fn <- codyna:::changepoint_best_split_
  # Line 92: n < 2 * min_segment
  result <- split_fn(c(1, 2, 3), "mean", 5)
  expect_null(result)
})

# --- Internal helper: changepoint_cusum_ max_cp limit (line 145) ------------

test_that("changepoint_cusum_ returns empty when max_cp is 0", {
  cusum_fn <- codyna:::changepoint_cusum_
  set.seed(42)
  x <- c(rnorm(50, 0), rnorm(50, 5))
  # Line 145: max_cp <= 0 returns integer(0)
  result <- cusum_fn(x, "mean", 1, 5, 0L, 0L)
  expect_length(result, 0L)
})

test_that("changepoint_cusum_ budget decrements after left recursion", {
  cusum_fn <- codyna:::changepoint_cusum_
  # Line 166: new_max decremented by left_cp count
  # Use multi-shift data with small penalty and limited max_cp
  set.seed(42)
  x <- c(rnorm(40, 0), rnorm(40, 10), rnorm(40, -5))
  result <- cusum_fn(x, "mean", 0.5, 5, 2L, 0L)
  expect_true(length(result) <= 2L)
})

# --- Internal helper: changepoint_binseg_ edge cases (lines 190-193, 204) ---

test_that("changepoint_binseg_ returns empty for short segment", {
  binseg_fn <- codyna:::changepoint_binseg_
  # Line 190: n < 2 * min_segment
  result <- binseg_fn(c(1, 2, 3), "mean", 1, 5, NULL, 0L)
  expect_length(result, 0L)
})
...
test_that("changepoint_binseg_ returns empty when max_cp is 0", {
  ...
  x <- c(rnorm(50, 0), rnorm(50, 5))
  # Line 191: max_cp <= 0
  result <- binseg_fn(x, "mean", 1, 5, 0L, 0L)
  expect_length(result, 0L)
})

test_that("changepoint_binseg_ handles NULL best_split result", {
  binseg_fn <- codyna:::changepoint_binseg_
  # Line 193: result is NULL from best_split (segment barely meeting 2*min_segment
  ...
  # If the gain is below penalty, it returns integer(0) at line 194
  result <- binseg_fn(rep(0, 10), "mean", 1000, 5, NULL, 0L)
  expect_length(result, 0L)
})

test_that("changepoint_binseg_ budget decrements after left recursion", {
  binseg_fn <- codyna:::changepoint_binseg_
  # Line 204: new_max decremented
  set.seed(42)
  x <- c(rnorm(40, 0), rnorm(40, 10), rnorm(40, -5))
  result <- binseg_fn(x, "mean", 0.5, 5, 2L, 0L)
  expect_true(length(result) <= 2L)
})

# --- Internal helper: changepoint_pelt_ edge cases (lines 230, 282, 288-299) -

test_that("changepoint_pelt_ returns empty for short segment", {
  pelt_fn <- codyna:::changepoint_pelt_
  # Line 230: n < 2 * min_segment
  result <- pelt_fn(c(1, 2, 3), "mean", 1, 5, NULL)
  expect_length(result, 0L)
})

test_that("changepoint_pelt_ returns empty when no changepoints found", {
  pelt_fn <- codyna:::changepoint_pelt_
  # Line 282: cps length == 0 (very high penalty, no split worthwhile)
  set.seed(42)
  x <- rnorm(50, 0, 1)
  result <- pelt_fn(x, "mean", 1e6, 5, NULL)
  expect_length(result, 0L)
})

test_that("changepoint_pelt_ enforces max_cp by pruning excess changepoints", {
  pelt_fn <- codyna:::changepoint_pelt_
  # Lines 288-299: max_cp truncation of PELT results
  set.seed(42)
  # Data with 3 clear shifts but max_cp = 1
  x <- c(rnorm(60, 0, 0.5), rnorm(60, 10, 0.5), rnorm(60, -5, 0.5))
  # Low penalty so PELT finds multiple changepoints
  result_full <- pelt_fn(x, "mean", 1, 5, NULL)
  expect_true(length(result_full) >= 2L)
  # Now restrict to max_cp = 1
  result_limited <- pelt_fn(x, "mean", 1, 5, 1L)
  expect_equal(length(result_limited), 1L)
})

# --- detect_cpts: max_changepoints validation (lines 576-579) ----------------

test_that("detect_cpts validates max_changepoints parameter", {
  set.seed(42)
  x <- c(rnorm(100, 0, 1), rnorm(100, 5, 1))
  # Lines 576-579: check_range on max_changepoints
...
# --- detect_cpts: changepoints too close filtering (line 619) ----------------

test_that("changepoints too close to each other are filtered", {
  # Line 619: keep[i] <- FALSE when consecutive cps are too close

  # Use a very small min_segment with crafted data that produces close cps
  set.seed(42)
  x <- c(rnorm(30, 0, 0.3), rnorm(30, 10, 0.3), rnorm(30, -5, 0.3),
         rnorm(30, 8, 0.3))
  # With pelt, low penalty, and min_segment = 5, it may produce close cps
...
test_that("variance type with binary_segmentation covers cost branches", {
  ...
  result <- detect_cpts(var_data, method = "binary_segmentation",
                        type = "variance", min_segment = 10L)
  expect_s3_class(result, "changepoint")
})
...
test_that("type both with pelt method covers cost branches", {
  ...
  result <- detect_cpts(both_data, method = "pelt", type = "both",
                        min_segment = 10L)
  expect_s3_class(result, "changepoint")
})
...
test_that("type variance with cusum covers cost branches", {
  set.seed(42)
  var_data <- c(rnorm(80, 0, 0.5), rnorm(80, 0, 5))
  result <- detect_cpts(var_data, method = "cusum", type = "variance",
                        min_segment = 10L)
  expect_s3_class(result, "changepoint")
})
...
test_that("summary returns change details when changepoints exist", {
  ...
  expect_true(all(c("location", "from_mean", "to_mean", "mean_shift") %in%
                    names(s$changes)))
})
...
# --- plot: fallback color for unknown state names (line 773) -----------------
...
test_that("plot handles state levels that fall back to cycling colors", {
  ...
  # Line 773: fallback color assignment
  p <- plot(result)
  expect_true(inherits(p, "gg") || inherits(p, "ggplot"))
})

# --- Lines 109, 193: dead code guards in best_split_ and binseg_ ------------
...
test_that("changepoint_best_split_ always finds a position when n >= 2*min_segment", {
  split_fn <- codyna:::changepoint_best_split_
  # Confirm that best_split always returns a non-NULL result for valid n
  x <- rep(0, 20)
  result <- split_fn(x, "mean", 5)
...
# --- Line 619: changepoints too close to each other -------------------------

test_that("cps too close are filtered via cusum with tight data", {
  ...
  x <- c(rnorm(12, 0, 0.1), rnorm(12, 50, 0.1), rnorm(12, -30, 0.1),
         rnorm(12, 80, 0.1), rnorm(12, -50, 0.1))
  # Use min_segment = 3 so close cps might occur
  result <- detect_cpts(x, method = "cusum", penalty = "manual",
                        penalty_value = 0.01, min_segment = 3L)
  expect_s3_class(result, "changepoint")
  locs <- attr(result, "changepoint_locations")
...
# --- Line 109: changepoint_best_split_ defensive guard (best_pos NULL) --------
# This guard at line 109 is unreachable because best_gain starts at -Inf and
...
test_that("changepoint_best_split_ always sets best_pos for valid n", {
  split_fn <- codyna:::changepoint_best_split_
  # Even constant data produces a valid split (gain = 0, pos set to first candidate)
  result <- split_fn(rep(0, 20), "mean", 5)
  expect_true(is.list(result))
...
# --- Line 193: changepoint_binseg_ defensive guard (NULL from best_split) -----
# This guard is unreachable because changepoint_binseg_ checks n < 2*min_segment
# at line 190 before calling best_split, and best_split always returns non-NULL
# when n >= 2*min_segment. We verify binseg returns the correct result when
# best_split gain is below penalty (line 194 branch instead).
...
test_that("changepoint_binseg_ returns empty when gain below penalty", {
  ...
  # This hits line 194 (gain <= pen) rather than line 193
  result <- binseg_fn(rep(0, 20), "mean", 1, 5, NULL, 0L)
  expect_length(result, 0L)
})

# --- Line 619: changepoint close-spacing filter (defensive guard) -------------
# All three methods (cusum, binseg, pelt) internally guarantee that adjacent
# changepoints are at least min_segment apart. The post-filter at line 619 is
...
test_that("all methods produce changepoints respecting min_segment spacing", {
  ...
  vapply(methods, function(m) {
    result <- detect_cpts(x, method = m, penalty = "manual",
                          penalty_value = 0.01, min_segment = 5L)
    locs <- attr(result, "changepoint_locations")
    if (length(locs) > 1L) {
      expect_true(all(diff(locs) >= 5L),
