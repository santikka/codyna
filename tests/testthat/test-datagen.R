...
test_that("compare_ts infers time index when time_col is NULL", {
  ...
  utils::capture.output(
    p <- compare_ts(
      data = df, ts_col = "value", time_col = NULL, state1 = "true_phase"
    ),
    type = "message"
  ) |>
    expect_error(NA)
...
test_that("compare_ts errors on missing columns", {
  ...
  expect_error(
    compare_ts(
      data = df, ts_col = "value", time_col = "time", state1 = "nonexistent"
    )
  )
})
...
test_that("generate_tipping_data ar model reaches saturation branch", {
  ...
  expect_true(
    all(vapply(tp[, var_cols], function(x) all(is.finite(x)), logical(1L)))
  )
})
...
test_that("compare_ts with facet_by adds facets", {
  ...
  expect_true(
    length(p$facet$params$facets) > 0L || !is.null(p$facet$params$facets)
  )
})
...
test_that("datagen_resilience_ errors for n_stable_levels = 0", {
  set.seed(0)
  expect_error(
    datagen_resilience_(200, 0, 100, 20)
  )
})
...
test_that("datagen_cluster_proportions_ uses provided proportions", {
  set.seed(0)
  props <- c(0.3, 0.3, 0.4)
  result <- datagen_cluster_proportions_(3, props, 0.1, TRUE)
...
test_that("datagen_cluster_proportions_ errors on wrong length proportions", {
  set.seed(0)
  expect_error(
    datagen_cluster_proportions_(3, c(0.5, 0.5), 0.1, TRUE)
  )
})
...
test_that("datagen_cluster_proportions_ errors when proportions don't sum to 1", {
  set.seed(0)
  expect_error(
    datagen_cluster_proportions_(3, c(0.3, 0.3, 0.3), 0.1, TRUE)
  )
})

test_that("datagen_cluster_proportions_ returns equal proportions when not randomized", {
  set.seed(0)
  result <- datagen_cluster_proportions_(4, NULL, 0.1, FALSE)
...
test_that("datagen_cluster_proportions_ warns when constraints unsatisfiable", {
  ...
  expect_warning(
    datagen_cluster_proportions_(3, NULL, 0.99, TRUE)
  )
})
