# --------------------------------------------------------------------------
# test-surrogates.R -- Tests for surrogate_test()
# --------------------------------------------------------------------------
...
test_that("method 'phase' works", {
  set.seed(42)
  surrogate_test(surr_data, n_surrogates = 19L, method = "phase",
                 window = 25L) |>
    expect_error(NA)
})
...
test_that("method 'shuffle' works", {
  set.seed(42)
  surrogate_test(surr_data, n_surrogates = 19L, method = "shuffle",
                 window = 25L) |>
    expect_error(NA)
})
...
test_that("method 'aaft' works", {
  set.seed(42)
  surrogate_test(surr_data, n_surrogates = 19L, method = "aaft",
                 window = 25L) |>
    expect_error(NA)
})
...
test_that("method 'block' works", {
  set.seed(42)
  surrogate_test(surr_data, n_surrogates = 19L, method = "block",
                 window = 25L) |>
    expect_error(NA)
})
...
test_that("metric 'ar1' works", {
  set.seed(42)
  surrogate_test(surr_data, n_surrogates = 19L, metric = "ar1",
                 window = 25L) |>
    expect_error(NA)
})
...
test_that("metric 'sd' works", {
  set.seed(42)
  surrogate_test(surr_data, n_surrogates = 19L, metric = "sd",
                 window = 25L) |>
    expect_error(NA)
})
...
test_that("metric 'skewness' works", {
  set.seed(42)
  surrogate_test(surr_data, n_surrogates = 19L, metric = "skewness",
                 window = 25L) |>
    expect_error(NA)
})
...
test_that("metric 'kurtosis' works", {
  set.seed(42)
  surrogate_test(surr_data, n_surrogates = 19L, metric = "kurtosis",
                 window = 25L) |>
    expect_error(NA)
})
...
test_that("bad method produces error", {
  expect_error(
    surrogate_test(surr_data, n_surrogates = 19L, method = "invalid",
                   window = 25L)
  )
})
...
test_that("bad metric produces error", {
  expect_error(
    surrogate_test(surr_data, n_surrogates = 19L, metric = "invalid",
                   window = 25L)
  )
})

# --------------------------------------------------------------------------
# n_surrogates controls number of surrogates
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# Attributes store settings
# --------------------------------------------------------------------------

test_that("attributes store analysis settings", {
  set.seed(42)
  st <- surrogate_test(surr_data, n_surrogates = 19L, method = "shuffle",
                       metric = "sd", window = 25L)
...
# --------------------------------------------------------------------------
# Internal helpers: surrogate_phase_ with odd-length series (lines 31-32)
# --------------------------------------------------------------------------
...
test_that("surrogate_phase_ handles odd-length series", {
  ...
  st <- surrogate_test(odd_data, n_surrogates = 19L, method = "phase",
                       window = 25L)
  expect_s3_class(st, "surrogate_test")
  expect_true(is.numeric(st$observed_tau))
})
...
# --------------------------------------------------------------------------
# surrogate_rolling_metric_ returns empty when window > length (line 101)
# --------------------------------------------------------------------------

test_that("surrogate_rolling_metric_ returns empty for window > data", {
  set.seed(42)
  result <- codyna:::surrogate_rolling_metric_(1:5, window = 10L, metric = "sd")
  expect_equal(length(result), 0L)
  expect_identical(result, numeric(0L))
})

# --------------------------------------------------------------------------
# surrogate_compute_metric_ returns NA for segment < 3 (line 116)
# --------------------------------------------------------------------------

test_that("surrogate_compute_metric_ returns NA for segment with < 3 values", {
  set.seed(42)
  result <- codyna:::surrogate_compute_metric_(c(1, 2), "ar1")
  expect_true(is.na(result))
  # Also test with all NAs
  result_na <- codyna:::surrogate_compute_metric_(c(NA_real_, NA_real_), "sd")
  expect_true(is.na(result_na))
})

# --------------------------------------------------------------------------
# skewness and kurtosis with zero sd (lines 133, 139)
# --------------------------------------------------------------------------

test_that("surrogate_compute_metric_ skewness returns NA for constant segment", {
  set.seed(42)
  # Constant segment has sd = 0
  result <- codyna:::surrogate_compute_metric_(rep(5, 10), "skewness")
  expect_true(is.na(result))
})

test_that("surrogate_compute_metric_ kurtosis returns NA for constant segment", {
  set.seed(42)
  result <- codyna:::surrogate_compute_metric_(rep(5, 10), "kurtosis")
  expect_true(is.na(result))
})

# --------------------------------------------------------------------------
# spectral_exponent metric (lines 143-150)
# --------------------------------------------------------------------------

test_that("metric 'spectral_exponent' works via surrogate_test", {
  set.seed(42)
  st <- surrogate_test(surr_data, n_surrogates = 19L,
                       metric = "spectral_exponent", window = 25L)
  expect_s3_class(st, "surrogate_test")
  expect_true(is.numeric(st$observed_tau))
...
test_that("surrogate_compute_metric_ spectral_exponent returns numeric", {
  set.seed(42)
  seg <- rnorm(50)
  result <- codyna:::surrogate_compute_metric_(seg, "spectral_exponent")
  expect_true(is.numeric(result))
  expect_equal(length(result), 1L)
})

# --------------------------------------------------------------------------
# hurst metric and surrogate_hurst_dfa_ (lines 153, 167-204)
# --------------------------------------------------------------------------

test_that("metric 'hurst' works via surrogate_test", {
  set.seed(42)
  st <- surrogate_test(surr_data, n_surrogates = 19L,
                       metric = "hurst", window = 25L)
  expect_s3_class(st, "surrogate_test")
  expect_true(is.numeric(st$observed_tau))
...
test_that("surrogate_hurst_dfa_ returns numeric for adequate length", {
  set.seed(42)
  x <- cumsum(rnorm(100))
  result <- codyna:::surrogate_hurst_dfa_(x)
  expect_true(is.numeric(result))
  expect_equal(length(result), 1L)
...
test_that("surrogate_hurst_dfa_ returns NA for too-short series", {
  ...
  result <- codyna:::surrogate_hurst_dfa_(c(1, 2, 3, 4, 5, 6, 7))
  expect_true(is.na(result))
})
...
# --------------------------------------------------------------------------
# surrogate_kendall_tau_ with fewer than 4 valid points (line 214)
# --------------------------------------------------------------------------

test_that("surrogate_kendall_tau_ returns NA for < 4 valid values", {
  set.seed(42)
  result <- codyna:::surrogate_kendall_tau_(c(1, 2, NA))
  expect_true(is.na(result))
  result2 <- codyna:::surrogate_kendall_tau_(c(NA, NA, NA))
  expect_true(is.na(result2))
})

# --------------------------------------------------------------------------
# surrogate_format_p_ edge cases (lines 241-242)
# --------------------------------------------------------------------------

test_that("surrogate_format_p_ handles NA", {
  set.seed(42)
  result <- codyna:::surrogate_format_p_(NA)
  expect_equal(result, "NA")
})

test_that("surrogate_format_p_ handles very small p-value", {
  set.seed(42)
  result <- codyna:::surrogate_format_p_(0.0001)
  expect_equal(result, "p < 0.001")
})

test_that("surrogate_format_p_ formats regular p-value", {
  set.seed(42)
  result <- codyna:::surrogate_format_p_(0.05)
  expect_equal(result, "p = 0.050")
})

# --------------------------------------------------------------------------
# Series too short error (lines 415-418)
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# NA p_value path when all surrogate taus are NA (lines 453-454)
# --------------------------------------------------------------------------
...
test_that("surrogate_test returns NA p_value when observed_tau is NA", {
  ...
  st <- surrogate_test(const_series, n_surrogates = 19L,
                       metric = "skewness", window = 10L)
...
# --------------------------------------------------------------------------
# Line 146: spectral_exponent with constant segment -> spec = 0 -> sum(valid) < 3
# --------------------------------------------------------------------------
...
test_that("surrogate_compute_metric_ spectral_exponent returns NA for constant segment", {
  ...
  # valid = (freq > 0 & spec > 0) has sum = 0 < 3, triggering line 146
  result <- codyna:::surrogate_compute_metric_(rep(5, 10), "spectral_exponent")
  expect_true(is.na(result))
})

# --------------------------------------------------------------------------
# Line 177: surrogate_hurst_dfa_ returns NA when length(scales) < 3
# --------------------------------------------------------------------------
...
test_that("surrogate_hurst_dfa_ returns NA when scales < 3 (n=20)", {
  ...
  # length(scales) = 2 < 3 -> returns NA at line 177
  result <- codyna:::surrogate_hurst_dfa_(seq_len(20))
  expect_true(is.na(result))
})

# --------------------------------------------------------------------------
# Line 200: surrogate_hurst_dfa_ returns NA when sum(valid) < 3 after fluct
# --------------------------------------------------------------------------
...
test_that("surrogate_hurst_dfa_ returns NA for constant data (zero fluctuation)", {
  ...
  result <- codyna:::surrogate_hurst_dfa_(rep(0, 40))
  expect_true(is.na(result))

  result2 <- codyna:::surrogate_hurst_dfa_(rep(1, 40))
  expect_true(is.na(result2))
})

# --------------------------------------------------------------------------
# Lines 181, 185: Defensive guards inside vapply of surrogate_hurst_dfa_
# These are unreachable through normal code paths because:
# - Line 181 (n_seg < 2): scales <= floor(n/4) guarantees n_seg >= 4
# - Line 185 (xx_var == 0): x_vals = 1:s with s >= 4 always has var > 0
# Test via direct manipulation to confirm the guards work.
# --------------------------------------------------------------------------
...
test_that("surrogate_hurst_dfa_ line 181/185 guards are robust (n=24)", {
  ...
  # xx_var for scale 4 = sum((1:4 - 2.5)^2) = 5 > 0, so line 185 not triggered
  # This exercises the full DFA computation including the inner vapply
  result <- codyna:::surrogate_hurst_dfa_(cumsum(rnorm(24, sd = 1)))
  expect_true(is.numeric(result))
  expect_equal(length(result), 1L)
})
