# --------------------------------------------------------------------------
# test-sensitivity.R -- Tests for sensitivity_ews()
# --------------------------------------------------------------------------
...
# ==========================================================================
# Multiple window sizes (default behavior)
# ==========================================================================
...
test_that("summary.sensitivity_ews does not error and returns list", {
  ...
  expect_true(all(
    c("metric", "tau_range", "tau_mean", "n_positive",
      "n_negative", "n_total", "most_robust",
      "least_robust", "consistent") %in% names(out)
  ))
})
...
# ==========================================================================
# Internal helper: sensitivity_detrend_ edge cases (lines 25, 27, 31-32)
# ==========================================================================
...
# ==========================================================================
# Internal helper: sensitivity_metric_ edge cases (lines 41, 45, 52, 61, 67, 72)
# ==========================================================================
...
# ==========================================================================
# sensitivity_rolling_: n_windows < 1 (lines 89-92)
# ==========================================================================
...
# ==========================================================================
# sensitivity_tau_: < 4 valid points (line 114), try-error (line 118)
# ==========================================================================
...
# ==========================================================================
# sensitivity_colors_: function coverage (lines 125-127)
# ==========================================================================
...
# ==========================================================================
# Default windows: w_min >= w_max branch (lines 282-284)
# ==========================================================================
...
# ==========================================================================
# Empty rolling result branch (lines 311-318)
# ==========================================================================
...
# ==========================================================================
# Summary: inconsistent (mixed signs) (lines 578-580)
# ==========================================================================
...
test_that("summary reports mixed signs when tau values differ in sign", {
  ...
  output <- capture.output(result <- summary(sa))
...
test_that("sensitivity_metric_ ar1 returns NA when ar.ols fails", {
  ...
  suppressWarnings(result <- metric_fn(x, "ar1"))
  expect_true(is.na(result))
})
...
# --- sensitivity_tau_: try-error when cor.test throws (line 118) ---

test_that("sensitivity_tau_ returns NA when cor.test throws (line 118)", {
  local_mocked_bindings(
    cor.test = function(...) stop("mock cor.test error"),
    .package = "stats"
  )
  tau_fn <- codyna:::sensitivity_tau_
  result <- tau_fn(c(1, 2, 3, 4, 5))
  expect_true(is.na(result))
})

# --- sensitivity_ews: empty rolling result creates zero-row tibble (lines 311-318) ---

test_that("sensitivity_ews creates empty tibble when rolling returns empty (lines 311-318)", {
  local_mocked_bindings(
